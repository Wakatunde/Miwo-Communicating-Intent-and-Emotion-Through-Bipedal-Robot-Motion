...
The logic of the dynamixel bridge differs from the Unitree bridge at startup, as this bridge reads the current position of all three servos and stores them as the initial tracked positions. The 3 servos have predefined idle position values that correspond to the head having zero degrees of roll, pitch, and yaw. This approach provides a known and previously calibrated neutral head position to which we can command the robot to return when not actively commanding it. If a servo fails for some reason and prevents torque from being enabled on startup, the bridge will automatically attempt to reboot the affected servo before retrying, further increasing the reliability of the startup sequence.
\newline
\textbf{Antenna control bridge}
\newline
The antenna bridge is responsible for controlling the two 9g servos that manage the antenna movement of the robot. Unlike the other two bridges that communicate directly with the onboard computer over a serial connection, the antenna servos are driven by an Adafruit PCA9685 16-channel PWM servo driver board. The PCA9685 is a dedicated PWM controller that generates the PWM signals required to position the servo motors. The PCA9685 communicates with an Arduino Nano over the I2C bus, the Arduino receives the position commands from the onboard Jetson over a USB serial connection. This chain of communication (Jetson - Arduino - PCA9685 - Servo) was chosen because the Jetson does not natively generate hardware PWM signals suitable for the servos used, and the Arduino provides a simple and widely available solution for bridging the communication. The bridge node subscribes to joint trajectory messages published to the antenna controller topic and translates the commands into PWM signals forwarded to the Arduino Nano. The Arduino runs a dedicated script that forwards the PWM signals received to the PCA9685, updating the servo positions. The antennas are not involved in the balance or inverse kinematics of the robot and act independently, allowing for movement to be layered on top of other motions. In the context of expressive motion, the antenna motion will serve as a secondary action previously mentioned in the animation principles.
\newline
\textbf{Eye Control Node}
\newline
The robots eyes are controlled from the onboard computer with an independent node. The node communicates with the two LCD displays over Wi-Fi, sending expression commands that correspond to a predefined set of different expressions like [happy, sad, squint, angry, idle]. The node also supplies the robot with realistic blinking by periodically publishing a blink command at random intervals with a randomized set of different blink animations, including [fast blink, single eye blink, slow blink, asymmetrical blink], which have different weights, to give a more lifelike blinking experience. In addition to sending out blink signals, the node also subscribes to an eye command topic, allowing the later discussed animation nodes to publish eye commands at specific parts of longer expressive motions.

\subsection{State Estimation}
\textbf{IMU pipeline}
the IMU pipeline is responsible for processing the raw sensor data from the Phidgets spatial IMU to produce a stable estimate of the robots orientation. The raw data from the IMU is published to the Phidgets ROS2 driver and then fed into a Madgwick filter that fuses the 2 sensor streams to produce an orientation estimate in quaternions. The Madgwick filter was chosen for its computational efficiency and accurate orientation tracking, matching the requirements of the system. The resulting orientation is then continuously published as a standard ROS2 message to a separate state node for further processing.
\newline
\textbf{Robot State Node}
The robot state node serves as the central state monitor for the robot. The node subscribes to the filtered IMU messages and the cartesian body state of the robot. This node also serves as a simple GUI for the user of the robot by displaying control modes, available actions, the published roll, pitch, and yaw in degrees, and the robots cartesian state in x, y, and z values. Upon startup, the node will also perform an automatic IMU calibration of the orientation by collecting IMU data over a period of 2 seconds while the robot sits in the calibration stand, setting the current orientation to be [0,0,0]. A manual recalibration service is also provided, allowing the user to reset the IMU offsets at any point during testing.
\subsection{Motion control}

\subsubsection*{Cartesian Body Controller}
The cartesian body controller serves as the central control node for the robot's motion. The controller subscribes to body pose commands expressed as a 6 DOF cartesian pose containing position [x,y,z] in millimeters and orientation [roll, pitch, yaw] in degrees and translates these into joint angle targets for the 10 actuators in the legs using the inverse kinematics solver. The resulting joint trajectories were published to the right and left leg controller topics, where the hardware bridge nodes relay the commands to the physical hardware. The current internal cartesian state of the robot is also continuously published to a dedicated topic to allow other nodes to read the current body position and use it as a base for generating relative motion offsets.
\subsubsection{Leveling Node}
The leveling node was implemented to provide automatic body leveling based on feedback from the available IMU data. It works by subscribing to the IMU orientation estimates and then computes corrective body twist commands to counteract any roll or pitch errors relative to the calibrated reference level. The correction is computed by using a quadratic dampening scheme to apply large corrections when the error is large and to slow down the correction as the error is minimized, preventing overshoots and oscillations. The leveling can be activated at any time by using a ROS2 service call, allowing it to be used selectively during operation. The leveling node was introduced to address an issue observed after the robot was removed from the calibration stand. Specifically, the robot’s legs exhibited slight compliance, causing a small but noticeable change in orientation when placed on solid ground without external support. Under the influence of gravity, this resulted in a minor sag in the robot’s posture. By implementing the leveling node, the system allows the user to invoke a leveling service after placement, enabling the calculation and adjustment of appropriate motor values to restore the desired orientation.

\subsection{Expressive motion Design}
The framework for generating expressive motion is built on a set of standalone ROS2 animation nodes, each responsible for generating a complete motion sequence to communicate the intended emotion or intent. All nodes combine a set of coordinated body motion,head motion, eye expressions, and antenna movement using different approaches. Each of the motion sequences can be better explained as animations, as each is hand-crafted using common animation principles. The animations were generated using different approaches, including the use of sinusoidal functions and separating longer animations into shorter stages and chaining them together. Resulting joint commands were translated to joint commands at runtime using the Inverse kinematics solver. 

All animations share a common design pattern. On startup, each node subscribes to the current cartesian body pose published by the body pose controller and utilizes the state as its base pose for starting the animation.. All subsequent motions are then computed relative to this base pose, allowing animations to be run from any robot configuration and avoiding positional jumps during startup. An additional delay is also applied after the base pose is collected to allow the robot time to settle into its current state.

The final designs of each animation were guided by the animation principles discussed in the background section of the thesis. Particular animation principles used include anticipation, slow-in and slow-out, secondary action, and exaggeration. Anticipation was deployed in multiple animations to telegraph upcoming actions; slow-in and slow-out were used to create smooth, natural motion, and a cosine ease interpolation was applied to all stage transitions. Secondary action is achieved in all the animations by coordinating different antenna and eye movements to complement and reinforce larger actions. Exaggeration is also applied by amplifying motion beyond a natural limit to further enhance the intended emotional response.
\subsection{Forward Kinematics}
The forward kinematics deployed in this project describe the translation from joint angles to the position and orientation of the robots end-effector, in this case, being the robots foot position relative to the base frame. By following the DH convention introduced in the background chapter, the kinematic chain of the robots legs consists of five joints: [hip yaw, hip roll, hip pitch, knee pitch, ankle pitch]. The forward kinematics transformation matrix can then be defined by multiplying the individual joint transformation matrices:

\begin{equation}
    T^0_n = A_1 A_2 \cdots A_n
\end{equation}

IN the context of this project, the forward kinematics were primarily used for simplistic state estimation. Given the current joint position of each actuator, the forward kinematics would compute the current foot position relative to the body. This information was used to update the robot state node, allowing the user to monitor the robots pose during operation and also to provide the base pose reference for the animation nodes used when calculating motion offsets.
\subsection{Inverse Kinematics}
The inverse kinematics solver is responsible for computing the joint angles required to place the feet at a desired position, given a commanded body pose. Rather than relying on a numerical iterative solver, an analytical closed-form solution was derived for the final robot. This approach was made possible due to the design choice of using a spherical hip joint, allowing the decoupling of the position and orientation of the leg. This approach was preferred as it could be solved in closed form without having to rely on more computationally expensive numerical methods, suiting the needs of the final system.

\textbf{coordinate convention and Frame transformations}
Body pose commands are expressed in the robots base frame and operate as a 6-DOF cartesian pose with position [x,y,z] expressed in millimeters and orientation [roll, pitch, yaw] in degrees. The body pose controller transforms these commands into per-leg foot position targets expressed relative to the hip's center, accounting for the robot's lateral and vertical hip offsets, as well as the geometric effects of the body's roll and pitch on the required foot placement. The relative hip-relative foot positions are then passed to the analytical IK solver for each leg independently. 
\begin{figure}[H]
    \centering
...
for its mirrored geometry. Body roll and pitch compensations 
are applied by the body pose controller prior to calling 
