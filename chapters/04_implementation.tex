...
The cartesian body controller serves as the central control node for the robot's motion.
\subsubsection{Leveling Node}
The leveling node was implemented to provide automatic body leveling based on the feedback from the available IMU data. It works by subscribing to the IMU orientation estimates and then computes corrective body twist commands to counteract any roll or pitch errors relative to the calibrated reference level. The correction is computed by using a quadratic dampening scheme to apply large corrections when the error is large and slows down the correction as the error is minimized preventing overshoots and oscillations. The leveling can be activated at any time by the use of a ROS2 service call allowing it to be used selectively during operation. The leveling node was implemented to solve an issue surrounding that the legs had a slight give after the robot was taken of the calibration stand. As the robot was placed on solid ground with no support the robots orientation would sag slight under the influence of gravity. By implementing the leveling node one could simply call the leveling service after 
